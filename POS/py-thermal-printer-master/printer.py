...
class ThermalPrinter(object):
    ...
    def __init__(self, heatTime=80, heatInterval=2, heatingDots=7, serialport=SERIALPORT):
        ...
        self.printer.write(chr(heatingDots))
        # heatTime Library default = 255 (max)
        self.printer.write(chr(heatTime))
        # Heat interval (500 uS = slower, but darker) default = 250
        self.printer.write(chr(heatInterval))
    ...
    def barcode_chr(self, msg):
        ...
        # Print barcode # 1:Abovebarcode 2:Below 3:Both 0:Not printed
            ...
            for i in xrange(0, 48 * chunkHeight):
                ...
                for xx in xrange(8):
                    ...
                    if pixel_value == 0:
                        byt += 1 << (7 - xx)
                        if output_png:
                            draw.point((counter % 384, round(
                                counter / 384)), fill=(0, 0, 0))
                    # it's white
                    else:
                        if output_png:
                            draw.point((counter % 384, round(
                                counter / 384)), fill=(255, 255, 255))
